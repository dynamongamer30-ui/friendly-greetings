    ...
    <div className="relative w-[84px] h-[84px] mx-auto">
      <svg width={size} height={size} className="-rotate-90">
        ...
        <circle
          ...
          style={{ transition: 'stroke-dashoffset 500ms ease', filter: 'drop-shadow(0 0 6px rgba(0,240,255,0.6))' }}
        />
      </svg>
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-sm font-bold text-[#00F0FF]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          {Math.round(value)}%
        </span>
      </div>
    </div>
      ...
      <div className="relative z-10 w-full max-w-md mx-auto px-4">
        <div className="glass-panel p-6 sm:p-8 space-y-6">
          ...
          <div className="w-full h-1.5 rounded-full bg-[rgba(255,255,255,0.06)] overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-[#00F0FF] to-[#7B2FFF] transition-all duration-500"
            ...
            {stages.map((s, i) => (
              <li key={i} className={`stage-row relative flex items-center gap-3 pl-3 ${s.status === 'done' ? 'stage-done' : ''}`}>
                ...
                <div className="shrink-0">
                  ...
                  {s.status === 'running' && <Loader2 className="w-5 h-5 text-[#00F0FF] animate-spin" />}
                ...
                <div className="flex-1 min-w-0">
                  <span style={{ fontFamily: "'Space Grotesk', sans-serif" }} className={`text-sm font-medium ${
                    ...
                    s.status === 'running' ? 'text-[#00F0FF]' : 'text-[#475569]'
                  }`}>
                    {s.label}
                  </span>
